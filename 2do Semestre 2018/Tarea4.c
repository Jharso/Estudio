...
int longCola(Cola);
void concatColas(Cola, Cola);

void invCola(Cola);
int existeElemento(Cola, int);
int igualesColas(Cola, Cola);
...
int main()
{
    ...
    printf("\nLa suma de los elementos en la pila es:  %d\n\n",sumaPila(pila1));
    ...
    igualesColas(cola1,cola2);
    imprimirCola(cola1);

    concatColas(cola1, cola2);
    imprimirCola(cola1);

    invCola(cola1);
    imprimirCola(cola1);

    existeElemento(cola1,44);
    existeElemento(cola1,5);
...
/*Hacer y retornar una copia de una cola. Esta operación debería colocarse como parte del TAD. La
complejidad de esta rutina es O( n ), donde n es la longitud de la cola.*/

Cola copiarCola(Cola cola)
{
    Cola resp=inicCola();
    Lista lista;
    for(lista=iniciaLista();longLista(cola)!=0;elimCola(cola))
    {
        printf("\n%d\n",infoCola(cola));
        anexaNodoLista(lista,infoCola(cola));        
    }
    for(primLista(lista);!finLista(lista);elimNodoLista(lista))
    {
        printf("\n%d\n",infoCola(lista));
        adicCola(cola,infoLista(lista));
        adicCola(resp,infoLista(lista));
    }
    destruirLista(lista);
    return resp;
}
...
int longCola(Cola cola)
{
    return longLista(cola);    
}

/*Concatenar dos colas, dejando el resultado en la primera de ellas. La complejidad de esta rutina es O( n ),
donde n es la longitud de la segunda cola.*/

void concatColas(Cola colaA, Cola colaB)
{
    Cola aux;
    for(aux=copiarCola(colaB);longLista(aux)!=0;elimCola(aux))
    {
        adicCola(colaA, infoCola(aux));
    }
    destruirCola(aux);
}
...
void fondoPila(Pila pila, int elemento)
{
    Lista lista;
    for(lista=iniciaLista();longLista(pila)!=0;elimPila(pila))
    {
...
int sumaPila(Pila pila)
{
    int aux;
    for(primLista(pila);!finLista(pila);sigLista(pila))
    {
        aux=aux+infoLista(pila);
    }
    return aux;
}

void invCola(Cola cola)
{
    Cola aux=copiarCola(cola);
    destruirCola(cola);
    printf("\ninvertir cola\n");
    for(cola=inicCola();longLista(aux)!=0;elimCola(aux))
    {
        adicPila(cola,infoCola(aux));
    }
    destruirCola(aux);
}

int existeElemento(Cola cola, int elemento)
{
    Cola aux;
    for(aux=copiarCola(cola);longLista(aux)!=0;elimCola(aux))
    {
        if(elemento==infoCola(aux))
        {
            printf("\nEl elemento %d si existe en la cola\n",elemento);
            return 1;
        }  
    }
    
    printf("\nEl elemento %d no existe en la cola\n",elemento);
    return 0;          
}

int igualesColas(Cola colaA, Cola colaB)
{
    printf("bandera");
    imprimirCola(colaA);
    Cola aux1=copiarCola(colaA);
    Cola aux2=copiarCola(colaB);
    while(longLista(aux1)!=0 && longLista(aux2)!=0)
    {
        
        if(infoCola(aux1)!=infoCola(aux2))
        {
            destruirCola(aux1);
            destruirCola(aux2);
